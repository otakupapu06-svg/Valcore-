...
function buildLampPosts() {
  ...
  positions.forEach(([x, z]) => {
    ...
    const headMat = new THREE.MeshStandardMaterial({
      color: 0xfff2c0, emissive: 0xffdd88, emissiveIntensity: 0.8
    });
    const head = new THREE.Mesh(new THREE.SphereGeometry(0.18, 8, 8), headMat);
    head.position.set(x, 3.2, z);
    scene.add(head);

    const lamp = new THREE.PointLight(0xffcc77, 0.6, 8, 2);
    lamp.position.set(x, 3.1, z);
    scene.add(lamp);
    scene.userData.lampLights.push(lamp);
  });
}
